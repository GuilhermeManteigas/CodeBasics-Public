...
if (getData("currentQuestionjs") == null){
	saveData("currentQuestionjs", 1, 300);
}
...
function saveInfoExperiment() {
	// We have to encodeURI to escape all the "" ' so the info is saved correctly
	saveData("experimentInfojs", encodeURIComponent(JSON.stringify(experimentArray)), 300);
}

function retrieveInfoExperiment() {
	if(getData("experimentInfojs")){
		experimentArray = JSON.parse(decodeURIComponent(getData("experimentInfojs")));
	}
}
...
$(document).on("keydown",function(e){
	saveCode();
	experimentArray[2][getData("currentQuestionjs")-1] = experimentArray[2][getData("currentQuestionjs")-1] + 1;
	// Check if Finish button should be enabled
	checkFinishBtn();
 });
...
function saveTime(){
	...
	experimentArray[1][getData("currentQuestionjs")-1] = experimentArray[1][getData("currentQuestionjs")-1] + ((currentTime - startTime)/1000);
		...
		<div class="card-body">
			...
			<div class="card" style="width: 450px">
				...
				<div class="card-body" style="user-select: none;">
					<h6><code>console.log('Hello my friend');</code></h6>
				</div>
			</div>
		</div>
	</div>`,
	`<div class="card">
		<div class="card-body">
			...
			<div class="card" style="width: 450px">
				...
				<div class="card-body" style="user-select: none;">
					<h6><code>console.log(10+9*2/6);</code></h6>
				</div>
			</div>
		</div>
	</div>`,
		...
		<div class="card-body">
			...
			<div class="card" style="width: 450px">
				...
				<div class="card-body" style="user-select: none;">
					...
					<h6><code>&nbsp console.log('y is less than 20');</code></h6>
					<h6><code>}else{</code></h6>
					<h6><code>&nbsp console.log('y is greater than or equal to 20');</code></h6>
					<h6><code>}</code></h6>
				</div>
			</div>
		</div>
	...
	`<div class="card">
		<div class="card-body">
			...
			<div class="card" style="width: 450px">
				...
				<div class="card-body" style="user-select: none;">
					<h6><code>function like(name){</code></h6>
					<h6><code>&nbsp console.log('I like ' + name);</code></h6>
					<h6><code>}</code></h6>
		...
		<div class="card-body">
			...
			<div class="card" style="width: 450px">
				...
				<div class="card-body" style="user-select: none;">
					...
					<h6><code>&nbsp console.log(i);</code></h6>
					<h6><code>}</code></h6>
				</div>
			</div>
		</div>
		...
		<div class="card-body">
			...
			<div class="card" style="width: 700px">
				...
				<div class="card-body" style="user-select: none;">
					...
					<h6><code>array.push(30);</code><code style="color:grey">&nbsp&nbsp&nbsp&nbsp&nbsp&nbsp&nbsp&nbsp&nbsp&nbsp<--- Adds the number 30 to the array</code></h6>
					<h6><code>array.splice(array.indexOf(18), 1);</code><code style="color:grey">&nbsp&nbsp&nbsp&nbsp&nbsp<--- Removes the number 18 from the array</code></h6>
					<h6><code>array.sort();</code></h6>
					<h6><code>console.log(array);</code></h6>
				</div>
			</div>
...
function saveCode(){
	experimentArray[0][getData("currentQuestionjs")-1] = codeEditor.getValue();
	saveInfoExperiment();
}
...
function selectBtns(){
	...
	const temp = document.querySelector('#Q'+getData("currentQuestionjs"));
	temp.classList.add("btn-warning");
	questionSection.innerHTML = questionList[getData("currentQuestionjs")-1];

	codeEditor.setValue(experimentArray[0][getData("currentQuestionjs")-1], 1);
...
q2Btn.addEventListener('click', () => {
	...
	saveCode();
	consoleLib.clearConsole();
	saveData("currentQuestionjs", 2, 300);
	selectBtns();
...
q3Btn.addEventListener('click', () => {
	...
	saveCode();
	consoleLib.clearConsole();
	saveData("currentQuestionjs", 3, 300);
	selectBtns();
...
q4Btn.addEventListener('click', () => {
	...
	saveCode();
	consoleLib.clearConsole();
	saveData("currentQuestionjs", 4, 300);
	selectBtns();
...
q5Btn.addEventListener('click', () => {
	...
	saveCode();
	consoleLib.clearConsole();
	saveData("currentQuestionjs", 5, 300);
	selectBtns();
...
q6Btn.addEventListener('click', () => {
	...
	saveCode();
	consoleLib.clearConsole();
	saveData("currentQuestionjs", 6, 300);
	selectBtns();
...
helpBtn.addEventListener('click', () => {
	experimentArray[3][getData("currentQuestionjs")-1] = experimentArray[3][getData("currentQuestionjs")-1] + 1;
...
executeCodeBtn.addEventListener('click', () => {
	// Create new console.log that saves logs to write later to console
	log = "";
	...
	// Run js local with eval()
	try {
		eval(codeEditor.getValue())
		consoleLib.printToConsole(log + '\n');
	} catch (e) {
		consoleLib.printToConsole(e + '\n');
	}
	...
	if(consoleeditor.getSession().getValue().replace(/\s/g, '').toLowerCase() == answerList[getData("currentQuestionjs")-1].replace(/\s/g, '').toLowerCase()){
